...
def walking():
    global kartesianX, kartesianY, kartesianZ
    global is_walking, walking_phases, last_timestamp
    global delay_legs, walking_direction, turn_direction
    
    # Only walk if a direction is set
    if walking_direction == 'none' and turn_direction == 'none':
        is_walking = False
        return
    ...
    if walking_direction == 'forward':
        ...
        for i in range(leg_count):
            kartesianX[i] = kartesian_ref[i,0] - y_delta
        return
    ...
    if turn_direction == 'left':
        kartesian[5] += np.radians(2)  # Rotate left
        return
    elif turn_direction == 'right':
        kartesian[5] -= np.radians(2)  # Rotate right
        return
